...
发现问题了吗？看完官方开始文档，我们只是配置好了测试环境，运行了官方模拟器，完全没有教怎么测试自己的处理器。🤣
...
- **Test Suites(测试套件)**: 根据不同ISA子集对测试进行分组，主要包含测试所需的**汇编程序代码**
- **The test signature(测试签名)**: 每一项测试结束时会生成一份签名，测试过程中处理器会向指定的内存区域写入数据，签名就是这些数据的值
...
其中DUT必须是个可执行程序，Python插件修改过程比较复杂，可以参考本项目的代码。
...
2. 复位，控制时钟信号等，完成仿真
3. 测试指令会使用一种被称为HTIF的机制与外接交互，从而通知测试结束，实际上就是往`tohost`地址写入1，需要监控这个写入从而结束仿真
...
对于小型的测试，我们可能希望有些代码段从0x00地址开始，节约仿真内存。在插件文件夹中有一个链接器脚本`[dutname]/env/link.ld`，你可以在此修改代码段的起始地址`.text.init`。需要注意的是，测试指令中写入操作依赖于**符号表**，如果你没有修改链接器脚本依然从内存的0x00地址加载代码段（代码段前通常是全0的无效数据），写入地址将会与**符号表**中地址不符，`tohost`,`begin_signature`,`end_signature`三个地址都需要减去代码段起始地址，代码段起始地址通常就是`rvtest_entry_point`，在**符号表**中可以查询到。
