...
class sail_cSim(pluginTemplate):
    ...
    def build(self, isa_yaml, platform_yaml):
        ...
        if shutil.which(self.sail_exe) is None:
            ...
            logger.error(
                "Sail has been updated to use sail_riscv_sim instead of riscv_sim_rv(32/64)d. Please make sure that you have the latest version of Sail installed."
            )
            raise SystemExit(1)
    ...
    def runTests(self, testList, cgf_file=None, header_file=None):
        ...
        for file in testList:
            ...
            try:
                sail_config = subprocess.run(
                    ["sail_riscv_sim", "--print-default-config"],
                    check=True,
                    text=True,
                    capture_output=True,
                )
            ...
            if pmp_flags == "" or pmp_flags == {}:
                sail_config["memory"]["pmp"]["grain"] = 0
                sail_config["memory"]["pmp"]["count"] = 0
            else:
