...
module AF_GPIO_LBUS #(
    ...
    parameter bit [31:0] FUNCT_IN_MASK[FUNCT_IN_COUNT],
    parameter int FUNCT_OUT_COUNT = 1,
    parameter bit [31:0] FUNCT_OUT_MASK[FUNCT_OUT_COUNT]
) (
    input gpio_clk,
    xt_lbus_if.port lb,
  ...
  // 方向寄存器0:输入  1:输出
  logic [COUNT-1:0] gpio_dir_reg = 0;

  // 数据寄存器
  logic [COUNT-1:0] gpio_out_data_reg;

  // GPIO控制
  logic [COUNT-1:0] gpio_out_af_en;
  logic [COUNT-1:0] gpio_out_af_val;
  logic [COUNT-1:0] gpio_in_reg;
  logic [COUNT-1:0] gpio_out_reg;
  ...
  generate
    for (genvar i = 0; i < COUNT; ++i) begin : gen_assign_gpio
  ...
  funct_af_t [FUNCT_IN_COUNT-1:0] funct_in_af_reg;
  generate
    for (genvar i_af = 0; i_af < FUNCT_IN_COUNT; ++i_af) begin : gen_af_funct_in
      // 32映射8
  ...
  funct_af_t [FUNCT_OUT_COUNT-1:0] funct_out_af_reg;
  always_comb begin
    int re;
    int gpio_index;
    ...
    for (int i = 0; i < COUNT; ++i) begin
      ...
      gpio_out_af_en[i] = 0;
      gpio_out_af_val[i] = 0;
      for (int o_af = 0; o_af < FUNCT_OUT_COUNT; ++o_af) begin
        re = 0;
        gpio_index = 0;
  ...
  always_ff @(posedge lb.clk) begin
    if (lb.wen) begin
      if (lb.addr[3:0] == BASE_ADDR_DIR) begin
        gpio_dir_reg <= lb.wdata[COUNT-1:0];
      end else if (lb.addr[3:0] == BASE_ADDR_DATA) begin
        gpio_out_data_reg <= lb.wdata[COUNT-1:0];
